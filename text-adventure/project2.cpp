...
#include <string>
#include <fstream>
//#include "gameutils.cpp" //for debugger
#include "gameutils.h"
using namespace std;
...
string makeExitList(room thisRoom) {
	...
	exitList[exitList.length() - 1] = (char)'.'; // change last comma to a period
	return exitList;
}
...
void doLook(room area, bool length) {
	...
	if (length == true) {
		...
		if (area.numItems == 0) {
			...
			cout << "You notice the following: " << area.items[area.numItems -1].shortDesc << ".\n";
		else {
		...
		if (area.numItems == 0) {
			...
			cout << "You notice the following: " << area.items[area.numItems-1].longDesc << ".\n";
		else {
			for (int i = area.numItems; i > 0; i--) {
